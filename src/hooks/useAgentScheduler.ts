...
export const useAgentScheduler = ({
  ...
  useEffect(() => {
    ...
    if (status === 'running' && sessionId && !isInitializedRef.current) {
      if (isJustStarted) {
        // Только что нажали кнопку (флаг свежий) - запускаем задачу
        console.log('✨ Ручной запуск кнопкой: запускаем задачу и планировщик');
        scheduleNextRun(true);
      } else {
        // Перезагрузка страницы, навигация или фоновый режим - только настраиваем интервал, БЕЗ запуска
        console.log('🔄 Фоновый режим / Навигация: планировщик работает, задача НЕ запускается');
